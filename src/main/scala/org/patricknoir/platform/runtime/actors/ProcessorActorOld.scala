...
/**
  ...
  */
class ProcessorActorOld[T](processor: Processor[T]) extends PersistentActor with ActorLogging { //with AtLeastOnceDelivery
  ...
  //TODO: as @jose.escanciano suggested use stash rather than implement internal queues of pending commands
...
object ProcessorActorOld {
  def props(processor: Processor[_]): Props = Props(new ProcessorActorOld(processor))
